...
export function add(
	...
	if (
		...
		return evaluator.sumElements(
			...
			galoisKeys,
			FHEModule.getSealSchemeType()
		)!;
	} else if (operands.length === 2) {
		if (
			operands.every(
				(operand) =>
					operand.data.instance.constructor.name === "Ciphertext"
			)
		) {
			...
			indexOfTypesMap.set(
				...
				operands.find(
					(operand) =>
						operand.data.instance.constructor.name === "Plaintext"
				)!.data
			);
			indexOfTypesMap.set(
				"Ciphertext",
