import { OperandTypes, CalculatorOperand } from 'breadwinner';
...
export function add(
    ...
    if (
        ...
        if (operands.every((operand) => operand.data.instance.constructor.name === 'Ciphertext')) {
            return evaluator.add(operands[0].data as CipherText, operands[1].data as CipherText)!;
        } else {
            const indexOfTypesMap = new Map<'Plaintext' | 'Ciphertext', PlainText | CipherText>();
            indexOfTypesMap.set(
                'Plaintext',
                operands.find((operand) => operand.data.instance.constructor.name === 'Plaintext')!
                    .data
            );
            indexOfTypesMap.set(
                'Ciphertext',
                operands.find((operand) => operand.data.instance.constructor.name === 'Ciphertext')!
                    .data
            );
...
export function subtract(evaluator: Evaluator, ...operands: CalculatorOperand[]): CipherText {
    if (
        operands.length === 2 &&
        ...
        return evaluator.subPlain(operands[0].data as CipherText, operands[1].data as PlainText)!;
    } else {
        throw new Error('Unsupported operand types.');
    }
}
